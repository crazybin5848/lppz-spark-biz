...
class ExportHanaIncreDataHandler extends Serializable{
  ...
  def exportHaNaTable(appName: String,bean:Hive2DbmsBean,sc:SparkContext,lowerBound:Long,upperBound:Long,ds:String)={
    val sqlContext = new SQLContext(sc)
    ...
    var fileName= bean.getHdfsUrl + "/tmp/hanahive/" + bean.getHiveSchema + "/" + bean.getHiveTableName + "/"+ ds +"/"+ lowerBound + "_" + upperBound
    df.rdd.map { r =>
        {
          val s: StringBuilder = new StringBuilder("")
          for (j <- Range(0, r.length)) {
